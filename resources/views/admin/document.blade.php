                ...
                <div class="modal-content">
                    ...
                    <form id="addDocument" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <input type="file" class="form-control form-control-alt" name="document" required></br>
                            <p>Giới hạn kích thước file tài liệu là 50MB</p>
                        </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            $(document).on('click', '.editbutton', function() {
                var id = $(this).data('id');
                $('#document_id').val(id); // Đặt giá trị của trường ẩn
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            // Hiển thị tên file tài liệu hiện tại
                            // Bạn cần thêm một trường để hiển thị tên file tài liệu hiện tại
            ...
            $('#editDocument').submit(function(e) {
                ...
                var formData = new FormData(this); // Sử dụng FormData để gửi file

                $.ajax({
                    url: "{{ route('editDocument') }}",
                    type: "POST",
                    data: formData,
                    processData: false, // Đặt processData và contentType thành false để gửi file
                    contentType: false,
                    success: function(response) {
                        if (response.success) {
                            location.reload();
            ...
            $("#deleteDocument").submit(function(e){
                ...
                $.ajax({
                    url:"{{ route('deleteDocument') }}", // Sửa route
                    type:"POST",
                    data:formData,
                    success:function(data){
                        if(data.success == true){
